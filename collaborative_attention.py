...
class Co_SparseAttention(nn.Module):
    def __init__(
        self,
        config,
        num_sparse_heads,
        ...
        super(Co_SparseAttention, self).__init__()

        self.num_attention_heads = num_sparse_heads  # 注意力头数
        ...
        if config.hidden_size % num_sparse_heads != 0:
            raise ValueError(
                "Value dimension ({}) should be divisible by number of heads ({})".format(
                    self.all_head_size, num_sparse_heads
                )
            )

        if not use_dense_layer and self.all_head_size!=dim_output:
            raise ValueError(
                "Output dimension ({}) should be equal to value dimension ({}) if no dense layer is used".format(
                    dim_output, self.all_head_size
                )
            )
        ...
        self.dim_value_per_head = self.all_head_size // num_sparse_heads
        ...
        self.content_bias = nn.Linear(config.hidden_size, num_sparse_heads, bias=False)
        ...
        self.dense = (
            nn.Linear(self.all_head_size, dim_output) if use_dense_layer else nn.Sequential()
        )
    ...
    def forward(
        ...
        # print(attention_scores.shape)输出查看之后到这一步是没问题的，暂时阉割掉之后的部分直接trans一下呢？
        # Normalize the attention scores to probabilities.
        # attention_probs = nn.Softmax(dim=-1)(attention_scores)
        attention_probs = self.relu(attention_scores)#换成sparc里的relu

        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        # attention_probs = self.dropout(attention_probs)#跟着sparc注释掉
        ...
        context_layer = attention_probs.transpose(1, 2)
    ...
    def init_mixing_matrix(self, scale=0.2):
        mixing = torch.zeros(self.num_attention_heads, self.all_head_size)

        if self.mixing_initialization is MixingMatrixInit.CONCATENATE:
            # last head will be smaller if not equally divisible
            dim_head = int(math.ceil(self.all_head_size / self.num_attention_heads))
